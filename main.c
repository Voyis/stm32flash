...
int main(int argc, char *argv[]) {
  ...
  if (action == ACT_READ) {
    ...
    // TODO: It is possible to write to non-page boundaries, by reading out
    // flash
    ...
    again:
      ...
      if (verify) {
        ...
        for (r = 0; r < len; ++r)
          if (buffer[r] != compare[r]) {
            if (failed == retry) {
              fprintf(stderr,
                      "Failed to verify at address 0x%08x, expected 0x%02x and "
                      "found 0x%02x\n",
      ...
      fprintf(diag, "\rWrote %saddress 0x%08x (%.2f%%) ",
              verify ? "and verified " : "", addr, (100.0f / size) * offset);
      fflush(diag);
    }
...
int parse_options(int argc, char *argv[]) {
  ...
  while ((c = getopt(argc, argv, "a:b:m:r:w:e:vn:g:jkfcChuos:S:F:i:d:R")) !=
         -1) {
    switch (c) {
    case 'a':
...
void show_help(char *name) {
  fprintf(
      ...
      "	-d		Path to gpio for switching DE/RE for 485 half duplex "
      "programming\n"
